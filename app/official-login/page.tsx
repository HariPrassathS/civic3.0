...
// =============================================================================
// Direct gateway for Tamil Nadu State Government Officials and Engineers.
...
// Direct 1-click authentication into executive dashboards.
...
export default function OfficialPortalLoginPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col justify-between selection:bg-emerald-500 selection:text-white relative overflow-hidden transition-colors">
      ...
      {/* Main Gateway Content: Direct Official Directory */}
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-6 py-8 flex flex-col justify-center">
        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
          {/* Top Status & Intro Banner */}
          <div className="p-5 sm:p-6 rounded-3xl bg-white/90 dark:bg-slate-900/85 border border-slate-200 dark:border-slate-800/90 backdrop-blur-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-xl">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                  Click any official role below to instantly log in and access your administrative governance dashboard.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-center justify-between">
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-900 dark:text-slate-100 placeholder:text-slate-400 focus:outline-none focus:border-emerald-500 transition-colors"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredOfficers.map((officer) => {
              ...
              return (
                <div
                  key={officer.role}
                  className="p-5 rounded-3xl bg-white dark:bg-slate-900/75 hover:bg-slate-50 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-emerald-500/50 transition-all flex flex-col justify-between gap-4 shadow-lg group relative overflow-hidden"
                >
                  {/* Top Tier Tag */}
                  ...
                  {/* Meta Details */}
                  <div className="space-y-1.5 bg-slate-50 dark:bg-slate-950/60 p-3 rounded-2xl border border-slate-200 dark:border-slate-800/80 text-[11px]">
                    <div className="flex items-center justify-between text-slate-700 dark:text-slate-300">
                      <span className="text-slate-400 dark:text-slate-500">Designation:</span>
                      <span className="font-semibold text-slate-900 dark:text-white">{officer.title}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-700 dark:text-slate-300">
                      <span className="text-slate-400 dark:text-slate-500">Jurisdiction:</span>
                      <span className="font-medium text-emerald-600 dark:text-emerald-400">{officer.jurisdiction}</span>
                    </div>
                    <div className="flex items-center justify-between text-slate-700 dark:text-slate-300">
                      <span className="text-slate-400 dark:text-slate-500">Gov Email:</span>
                      <span className="font-mono text-[10px] text-slate-500 dark:text-slate-400">{officer.email}</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold shadow-md shadow-emerald-950/20 transition-all flex items-center justify-center gap-2 active:scale-[0.99] disabled:opacity-50 cursor-pointer"
                  >
                    {isCurrentAction ? (
      ...
      {/* Footer Notice */}
      <footer className="border-t border-slate-200 dark:border-slate-800/80 bg-white/80 dark:bg-slate-950/90 py-4 px-4 text-center text-[11px] text-slate-500">
